...
impl SelectByPath {
	fn matches_path(&self, path: &metric::DevicePath) -> bool {
		...
		match self.match_device_type.as_ref() {
			Some(p) => {
				if !p.matches_with(&path.device_type, MATCH_OPTIONS) {
					trace!(
						"select by path rejected {:?} because the device type did not match {:?}",
						path,
						self.match_device_type
					);
					return self.invert;
				}
			}
		...
		match self.match_instance.as_ref() {
			Some(p) => {
				if !p.matches_with(&path.instance, MATCH_OPTIONS) {
					trace!(
						"select by path rejected {:?} because the instance did not match {:?}",
						path,
						self.match_instance
					);
					return self.invert;
...
impl Filter for Calc {
	fn process_readout(&self, mut input: payload::Readout) -> Option<payload::Readout> {
		if !self.predicate.matches_readout(&input) {
			trace!(
				"calc skipping {:?} because it was rejected by the predicate",
				input
			);
			return Some(input);
		}

		let ctx = script::Context::new(script::BoxCow::<'_, dyn script::Namespace>::wrap_ref(
			&input.components,
		));
...
impl Filter for Map {
	fn process_readout(&self, mut input: payload::Readout) -> Option<payload::Readout> {
		if !self.predicate.matches_readout(&input) {
			trace!(
				"map skipping {:?} because it was rejected by the predicate",
				input
			);
			return Some(input);
		}
		...
		for (k, v) in input_mut.components.drain(..) {
			...
			let ctx =
				script::Context::new(script::BoxCow::<'_, dyn script::Namespace>::wrap_ref(&ns));
...
impl Filter for KeepComponent {
	fn process_readout(&self, mut input: payload::Readout) -> Option<payload::Readout> {
		...
		if input.components.contains_key(&self.component_name) {
			...
			input_mut
				.components
				.retain(|(k, _v)| *k == self.component_name);
		}
		Some(input)
	}
}
