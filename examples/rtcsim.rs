...
impl Imperfection<u16> for OffsetError<u16> {
	fn mess_with(&mut self, state: &mut State, v: u16) -> u16 {
		v.wrapping_add(self.offset)
	}
}

impl Imperfection<DateTime<Utc>> for OffsetError<Duration> {
	fn mess_with(&mut self, state: &mut State, v: DateTime<Utc>) -> DateTime<Utc> {
		v + self.offset
	}
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let mut driver = Driver::new(
		vec![
			...
			//Box::new(AccumJitterError{distr: rand_distr::Normal::new(0.0f64, 0.1f64).unwrap(), accum: 0.0}),
	...
	let mut mappers: Vec<Box<dyn RTCifier>> = vec![
		...
		// Box::new(RangeRTCv2::default()),
		Box::new(RangeRTCLiori::default()),
	...
	for (clock, real, rtc, ctr) in driver {
		...
		println!("{} {} {}", real.timestamp_millis(), rtc.timestamp(), ctr);
	}
	Ok(())
}
